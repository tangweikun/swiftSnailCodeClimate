      ...
      <SelectorWrapper>
        <Selector>
          {filteredCorpus.map(_corpus => (
            <Option
              key={_corpus._id}
              onClick={() => onCorpusSelect(_corpus)}
              isSelected={_corpus._id === selectedAssessment}
            >
              <OptionContent>{_corpus.value}</OptionContent>
              {_corpus.createdBy && (
                <Trash
                  type="delete"
                  onClick={(e) => {
...
`
const OptionContent = styled.div`flex: 1 1;`
